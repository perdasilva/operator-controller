...
func (r *RegistryV1HelmChartProvider) Get(bundleFS fs.FS, ext *ocv1.ClusterExtension) (*chart.Chart, error) {
    ...
    // TODO: the need to get the underlying bundle in order to extract its annotations
    //   should go away once with have a bundle interface that can surface the annotations independently of the
    //   the underlying bundle format...
    rv1, err := source.FromFS(bundleFS).GetBundle()
    if err != nil {
        return nil, err
    }
