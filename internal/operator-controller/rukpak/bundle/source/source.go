...
func (f fsBundleSource) GetBundle() (bundle.RegistryV1, error) {
    ...
    if err != nil {
        if errors.Is(err, fs.ErrNotExist) {
            return reg, errors.New("malformed bundle: required file metadata/annotations.yaml not found")
        }
        return reg, err
    }
    ...
    if annotationsFile.Annotations.PackageName == "" {
        return reg, errors.New("malformed bundle: operators.operatorframework.io.bundle.package.v1 annotation not found in metadata/annotations.yaml")
    }
    reg.PackageName = annotationsFile.Annotations.PackageName

    const manifestsDir = "manifests"
    foundCSV := false
    s, err := fs.Stat(f.FS, manifestsDir)
    if os.IsNotExist(err) || !s.IsDir() {
        return reg, errors.New("malformed bundle: manifests directory not found")
    }
    if err != nil {
        return reg, fmt.Errorf("error loading bundle: %w", err)
    }
    if err := fs.WalkDir(f.FS, manifestsDir, func(path string, e fs.DirEntry, err error) error {
        ...
        if err := result.Visit(func(info *resource.Info, err error) error {
            ...
            return fmt.Errorf("error loading bundle: could not parse objects in %q: %v", path, err)
        }
        return nil
    }); err != nil {
        return reg, err
    ...
    if !foundCSV {
        return reg, fmt.Errorf("malformed bundle: no ClusterServiceVersion found in %q directory", manifestsDir)
    }
