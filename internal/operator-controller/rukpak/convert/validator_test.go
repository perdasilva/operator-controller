...
func Test_RegistryV1BundleValidatorCorrectValidationFns(t *testing.T) {
	expectedValidationFns := []func(v1 *convert.RegistryV1) []error{
		...
		convert.CheckConversionWebhooksReferenceOwnedCRDs,
		convert.CheckConversionWebhookCRDReferenceUniqueness,
	}
...
func Test_CheckConversionWebhooksReferenceOwnedCRDs(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			errs := convert.CheckConversionWebhooksReferenceOwnedCRDs(tc.bundle)
			require.Equal(t, tc.expectedErrs, errs)
		})
	}
}
...
func Test_CheckConversionWebhookCRDReferenceUniqueness(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			errs := convert.CheckConversionWebhookCRDReferenceUniqueness(tc.bundle)
			require.Equal(t, tc.expectedErrs, errs)
		})
	}
}
