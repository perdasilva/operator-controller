...
import (
	...
	"github.com/operator-framework/operator-controller/internal/util/clusterextension"
)
...
func (h *Helm) Apply(ctx context.Context, contentFS fs.FS, ext *ocv1.ClusterExtension, objectLabels map[string]string, storageLabels map[string]string) ([]client.Object, string, error) {
	chrt, err := convert.RegistryV1ToHelmChart(ctx, contentFS, ext.Spec.Namespace, []string{clusterextension.GetWatchNamespace(ext)})
	if err != nil {
		return nil, "", err
	}
