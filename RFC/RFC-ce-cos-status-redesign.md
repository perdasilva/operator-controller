...
**Proposed**: `Ready`, `Progressing`, `Version`, `Rollout`, `Target`, `Age`

| Column | JSONPath | Rationale |
|--------|---------|-----------|
| Ready | `.status.conditions[?(@.type=='Ready')].status` | Primary health signal — the first thing users check |
| Progressing | `.status.conditions[?(@.type=='Progressing')].reason` | What the controller is doing — **shows the reason, not the boolean**. More informative: `Succeeded` (done), `RollingOut` (active), `Retrying` (error, retrying), `Blocked` (terminal), `InvalidConfiguration` (fix spec), `ProgressDeadlineExceeded` (timed out) |
...
`Installed Bundle` (the bundle name) is dropped because it's rarely needed at a glance — the CE name itself identifies the extension, and the version is more actionable. The `Installed` condition column is replaced by `Ready`, which is a more useful signal. The `Progressing` column shows the **reason** rather than the boolean status — the reason is strictly more informative (the boolean can be inferred from the reason), and it answers "should I worry?" directly. The `Rollout` and `Target` columns provide upgrade visibility without requiring `kubectl describe`.
...
- `clusterextension_types.go`:
  ...
  - Update print columns (Ready, Progressing, Version, Rollout, Target, Age)
...
4. **Better kubectl experience**: Print columns show what matters — `Ready` and `Progressing` give an at-a-glance health summary, and `Version` shows what's installed.
